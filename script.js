/* ==========================================
   ...
   PWA | Firebase Real-time | Dark mode
========================================== */

// ── FIREBASE CONFIG ────────────────────────
// Bu yerga o'z Firebase config'ingizni kiriting
// console.firebase.google.com → loyiha → Project settings → Your apps
...
const DB_PATH = "spiska/products"; // Realtime DB yo'li
...
function initFirebase() {
  ...
  // Real-time listener — boshqa qurilmadan o'zgarish bo'lsa darhol yangilaydi
  dbRef.on(
    "value",
    (snapshot) => {
      if (isSyncing) return; // o'zimiz yozayotganda o'zimizni echo qilmasin
      ...
      if (val === null) {
        ...
        // object formatida kelsa array ga aylantir
...
async function saveAndSync() {
  ...
  try {
    ...
    // Kichik delay — o'z echo'miz kelganda isSyncing=true bo'lsin
    setTimeout(() => {
      isSyncing = false;
    }, 500);
  }
...
// ── INIT ───────────────────────────────────
setSyncStatus("syncing");
// Firebase SDK yuklanganidan keyin ishga tushadi (index.html dagi onload)
